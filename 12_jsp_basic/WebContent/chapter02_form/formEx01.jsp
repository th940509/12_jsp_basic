...
<script>
	function sendData() {
		...
		// document.form태그의 name.submit();
		document.f.submit();
	}
</script>
</head>
<body>
	<!-- action 속성에 데이터를 전송할 페이지를 작성한다. -->
	<!-- method 속성에 데이터를 전송할 방식을 작성한다.(기본값은 get 방식) -->
	<form action="formEx01Pro.jsp" method="post" name="f">
		<fieldset>
			<!-- form태그 안에 있는 name속성의 이름으로 데이터가 서버로 전송된다. -->
			<p>좋아하는 언어: <input type="text" name="languages"></p>
			<p>좋아하는 이유: <br>
				<textarea rows="5" cols="30" name="reason">
				</textarea>
			</p>
			<p>사용가능한 언어의 개수: <input type="text" name="languageCnt"></p>
			<!-- 전송방법 1) submit타입을 이용하여 서버로 전송한다.(일반적인 방법) -->
			<input type="submit" value="입력완료">
			
			<!-- 전송방법 2) 자바스크립트를 이용하여 서버로 전송한다. -->
