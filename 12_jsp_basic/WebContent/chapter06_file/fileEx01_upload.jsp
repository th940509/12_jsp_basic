...
<%
	...
	// 파일의 저장 경로는 웹서비스 접근 경로가 아닌 물리적 실제 경로를 적어주어야 한다.
	...
	// multipart/form-data형식으로 전송되었기 때문에 request.getParameter가 불가능하고 MultipartRequest 객체의 getParameter메서드를 사용해야 한다.
	...
	if (files.hasMoreElements()) { // 다음 정보가 있으면
		...
		String originalFileName = multi.getOriginalFileName(element); // 사용자가 업로드 한 파일명을 반환
		String fileType         = multi.getContentType(element);      // 업로드된 파일의 타입을 반환
		long fileLength         = multi.getFile(element).length();    // 파일의 크기를 반환 (long 타입만 가능) * long : int보다 큰것
